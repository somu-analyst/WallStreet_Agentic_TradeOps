...
- Flow/positioning: `/uoa [live] [TICKERS]` vol/OI ≥2, DTE≥7 (tickers = same bar for just those names; filtered calls never write scanner fires). `live` = today's CBOE chain: with tickers it fetches now, without them it serves the newest `uoa_live` snapshot — `uoa_live_job` captures ~30 core names + book + watchlist every 30 min, 9:45–16:15 ET. `uoa_live`/`uoa_live_runs` are ACCRUING (never purged; no backfill possible) · `/building` new OI staged S/I/C.
