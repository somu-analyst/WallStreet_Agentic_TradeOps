...
- **Limit lockouts (AUTOMATED 2026-07-16):** a `Stop` hook (`.claude/settings.local.json` → `limit_guard.ps1`, async, 10-min throttle) checks the active 5h block via ccusage; past the threshold (`.claude/limit_threshold_tokens.txt`, 80M ≈ 95% of the largest block seen) it writes `[RESUME AFTER] <reset>` at the top of `NEXT.md` and registers a one-shot `ClaudeResume` scheduled task that reopens `claude --continue` here 3 min after reset. Manual fallback: `.\resume_after_limit.ps1 -At "HH:mm"`. Claude's part when throttling appears mid-session: fresh-start summary to `LOG.md`/`NEXT.md` + PLAN ticked to the exact subtask; on return `/standup` reads NEXT first — resume cold from docs, never replay the thread.
