...
- [ ] **`/debate` weight rebalance.** Re-checked 2026-07-24, found the premise half-wrong: the code comment right above `_AGENT_WEIGHTS` (line 2680-2685) already re-validated the "Technical t=+4.28" figure this PLAN.md line cited — it did NOT replicate (actual measured rank-IC was **-0.039** at fwd-10d, a mild REVERSAL, not the momentum edge originally claimed). Did NOT rebalance based on that stale number — would have been overfitting on evidence the codebase already flagged as wrong. What's still solid: Flow genuinely has zero edge (t=-0.07, not disputed) at the highest weight (1.2). What's missing: no reliable number for what Technical (or Position/Macro) SHOULD weigh instead — a proper agent-level backtest doesn't exist yet. Real next step, if wanted: build that backtest (same recipe as `.claude/rules/bot-conventions.md`'s "Signal validation recipe") before touching any weight, not before.
