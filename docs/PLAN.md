...
- [x] ~~BS next-day model insights → dashboard~~ — **DONE 2026-07-28**, user picked "both". (1) Est Open relabel: already done in an earlier session (both caption sites already say "backtested: NO edge over Now, trust the scenarios instead" — found already-shipped, not re-touched). (2) Liquidity gate on the scenario/Day L–H recipe: NEW `_scenario_liquidity_gate()` (vol≥100 from the latest captured `options_change` row + bid/ask spread≤8% when a live quote exists) wired into both Day L–H sites (flat view + by-stock view) — thin legs now show a `⚠️thin` tag and both captions explain what it means. Mid-quote IV anchoring was already in place (live bid/ask re-anchors IV, confirmed at the `_live_mid`/`_live_iv` block). Verified live via Playwright: zero exceptions, both captions render the new text, GOOG's currently-liquid position correctly shows no false-positive flag.
...
- [x] Skew25 panel IC — **RERUN 2026-07-28 at 17 dates (10 usable w/ fwd-5d window), gate cleared.** Sign flipped from the 6-date first read (now correctly negative = matches the downside hypothesis) across all 3 metrics tested, but only `pcoi` (put/call OI ratio) reaches significance: **skew25** rank-IC −0.048 (t=−1.57, not sig) · **pcvol** −0.019 (t=−0.88, not sig) · **pcoi** −0.058 (**t=−3.17, significant**). Caveats before trusting this: only 10 dates (thin), and 3 metrics were tested at once (multiple-comparison risk — finding 1/3 significant is close to what chance alone would produce). Promising, not conclusive — rerun once more skew_snapshot history accrues (target 25-30+ usable dates) before building any live scanner off `pcoi`. Script: session scratchpad `skew_backtest.py` pattern (per-date cross-sectional rank-IC vs fwd-5d return, liq>0.05 gate, mean+t-stat across dates).
...
- [ ] Dispersion (#14) — separate from the skew backtest above; still not built, no specific data gate identified yet.
...
| A9 | ~~Read-through / narrative as LINKS~~ | **DONE 2026-07-27** (commit 0349c62) — user clarified this meant "news headlines as hyperlinks." Audited every narrative surface quoting a headline: positions card, world news, OI ticker view, dashboard ticker news, catalysts geopolitical flags, `market_headlines` all already linked (mostly from the 2026-07-24 hyperlinks pass, commit 9129bb2). One real gap found: `/wrap`'s "THE CATALYST" quote discarded the URL yfinance's news item carries — fixed. |
| A10 | ~~Single consolidated alert message~~ | **DONE 2026-07-28** — `_status_push()` folds `position_monitor` + `intraday_alert` (the only two that ALWAYS fire on schedule regardless of content) into ONE message edited in place per trading day (`app_settings.status_msg_id`/`status_msg_date`, 5s edit-rate guard, sections drop off automatically on a new day). Scoped DOWN from the plan's literal "~5" list on purpose: `heat_streamer_alert`/`wan_streamer_alert`/`building_alert` are trigger-only (one push per NEW state-change, already deduped) — exactly the "genuine news... trigger alerts... should interrupt" case the plan itself says must stay separate, so they were left as-is. `position_monitor(force=True)` (manual button / adhoc trigger) still sends the full rich card with its keyboard immediately, unaffected. Verified LIVE: real `sendMessage` then real `editMessageText` (200 OK) against the actual bot/chat, combining both sections into one message, then cleaned up the test message. |
...
| A12 | ~~Dashboard: market overview collapsible dropdown at top~~ | **DONE 2026-07-27** (commit 7ca1296) — the 5-category card grid (Indices/Futures/Commodities/FX-Crypto/Bonds), the first big wall of scroll on the Market Overview page, is now a collapsed-by-default expander. Verified live via Playwright: collapsed shows no card text, opens cleanly with all data, zero exceptions either state. |
| A13 | ~~Portfolio Greeks as dropdown~~ | **Already DONE, stale row** — commit 22414cf ("Portfolio Greeks: collapsible expander, Net Delta in the header") shipped this earlier in the session; dashboard.py:14848 confirms a live `st.expander`. Just never ticked off. |
| A14 | ~~Wire PEAD into a command~~ | **DONE** — `/pead` registered (`pead_command`, telegram_bot_optimized.py:27577), stale audit line. |
| A15 | ~~Dashboard Portfolio page render-check~~ | **DONE 2026-07-27 — opened live via Playwright for the first time.** All 4 tabs (Open Positions/Performance/Suggestions/P&L Breakdown) render with zero exceptions; real GOOG position data, P&L, Greeks, closed-positions table (79 rows) all correct. Found one real but minor, self-healing bug: for ~10-20s right after navigating away from the default landing page (Next-Day Exit Planner), that page's `run_every=5` live-refresh fragments (`_gp_live_strip`/`_gp_live_tick`, dashboard.py:1337/1384) visibly bleed stale content into the newly-selected page before Streamlit's own reconciliation clears it (confirmed transient: leak present at 2s/10s, gone by 30s). Cosmetic only — no data corruption, no crash, self-resolves. NOT patched: the live-refresh fragments are a deliberate, carefully-profiled perf fix (commit 597343e, solved a real page-freeze bug) and touching their lifecycle without being certain of the exact Streamlit reconciliation mechanism risks reintroducing that freeze. Flagged as a new, separate, low-priority follow-up rather than guess-patched. |
...
| A18 | ~~SI buildup/covering validation vs baseline~~ | **DONE 2026-07-30 — unblocked early.** The "wait 2-3 months for live accrual" premise didn't need to hold: Nasdaq's public short-interest API gives a full ~1yr settlement-date history per ticker for free (`_si_fetch_history`), backfilled now instead of waiting (`_si_backfill`: 1590 rows, 64 tickers, 23-25 dates each, 2025-07-15→2026-07-15). Real backtest: SI %-change vs fwd 10d return, N=1462, rank-IC +0.017 (t=+0.65, p=0.52) — **no meaningful edge**, BUILDING (+1.36% fwd) ≈ COVERING (+1.13% fwd) ≈ baseline (+1.55%). Same "descriptive, not directional" verdict as `/building`/`/uoa`; `_si_read`'s docstring updated with the finding so the read is never oversold as a signal. |
