...
# The actual FTS5 index/ingestion (_rag_docs/_rag_index/search_notes_core) now lives in
# telegram_bot_optimized.py, not here (moved 2026-07-27) — this module already imports
# that one as a library, so it owning the corpus and this file delegating is the only way
# to avoid two ingestion paths without a circular import. This tool is now a thin wrapper.
@mcp.tool()
def search_notes(query: str, top_k: int = 6, source: str | None = None) -> dict:
    ...
    Indexes event write-ups, the news_feed, sentiment_log, 13F filings (edgar_13f), the
    macro/earnings event_catalog, trade journal + bookmarks, and the docs/*.md continuity
    logs (LOG/NEXT/PLAN) into a local SQLite FTS5 index, rebuilt on demand — no external
    API or embeddings, fully free and offline. Returns the top matches ranked by BM25,
    each with source, ref, date, title and a snippet.
    ...
    """
    return bot.search_notes_core(query, top_k=top_k, source=source)
