async def group_stock_detail(query, ticker):
    """Show all open option positions for a given stock (ticker) as a group, with P&L and details."""
    conn = get_conn()
    try:
    ...
    total_pnl = trades_df['unrealized_pnl'].sum() if 'unrealized_pnl' in trades_df else 0
    pnl_emoji = "🟢" if total_pnl >= 0 else "🔴"
    parts = [hdr(f"📦 {ticker} — All Options"),
             f"{pnl_emoji} <b>Unrealized P&L:</b> ${total_pnl:+,.0f}"]
    parts.append("\n<b>Legs:</b>")
    for _, trade in trades_df.iterrows():
        ...
        exp = str(trade["expiry"])
        entry = float(trade["entry_price"])
        cur = float(trade["current_price"]) if 'current_price' in trade and trade["current_price"] is not None else 0
        pnl = float(trade["unrealized_pnl"]) if 'unrealized_pnl' in trade and trade["unrealized_pnl"] is not None else 0
        side_emoji = "🟢" if qty > 0 else "🔴"
        parts.append(f"{side_emoji} #{tid} {ot} ${st:.0f} x{qty}  Exp:{exp}  Entry:${entry:.2f}  Now:${cur:.2f}  P&L:${pnl:+.0f}")
...
try:
    ...
    async def _reply_text_sanitized(self, text, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                return await _orig_reply(self, safe or text2, *args, parse_mode=None, **kwargs)
            except Exception:
                raise
    ...
    async def _send_message_sanitized(self, chat_id, text=None, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                return await _orig_send(self, chat_id=chat_id, text=safe, parse_mode=None, *args, **kwargs)
            except Exception:
                raise
    ...
    async def _send_photo_sanitized(self, chat_id, photo, caption=None, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                return await _orig_send_photo(self, chat_id=chat_id, photo=photo, caption=safe, parse_mode=None, *args, **kwargs)
            except Exception:
                raise
    ...
    async def _reply_photo_sanitized(self, photo, caption=None, *args, **kwargs):
        ...
        try:
            ...
            try:
                safe = orig_cap.replace('<', '&lt;').replace('>', '&gt;') if isinstance(orig_cap, str) else cap2
                return await _orig_reply_photo(self, photo, caption=safe, parse_mode=None, *args, **kwargs)
            except Exception:
                raise
...
def bs_greeks(S, K, T, r, sigma, opt="put"):
    ...
    if opt == "put":
        ...
        price = S * norm.cfd(d1) - K * np.exp(-r * T) * norm.cdf(d2)
        delta = norm.cdf(d1)
        theta = (-S * pdf1 * sigma / (2 * np.sqrt(T)) - r * K * np.exp(-r * T) * norm.cdf(d2)) / 365
...
async def market_overview(query):
    ...
    # ── Compact 4-col <pre> (mobile-safe ≈28 chars, no Dir/Note) ──
    hdrs = ["ST", "Name", "Price", "Chg%"]
    RIGHT = {2, 3}
    str_rows = []
    for sec, name, d, tag in all_rows:
        if d:
            str_rows.append([d["st"], name, d["px_s"], f"{d['chg']:+.2f}%"])
        else:
            str_rows.append(["[?]", name, "N/A", "---"])

    col_w = [max(len(hdrs[i]), max(len(r[i]) for r in str_rows)) for i in range(len(hdrs))]
    _j    = lambda i, v: v.rjust(col_w[i]) if i in RIGHT else v.ljust(col_w[i])
    sep   = "-+-".join("-" * w for w in col_w)
    now_s = datetime.now().strftime("%H:%M ET")

    tbl = [f"MKT {now_s}", " | ".join(_j(i, hdrs[i]) for i in range(len(hdrs))), sep]
    cur_sec = ""
    for (sec, name, d, tag), r in zip(all_rows, str_rows):
        if sec != cur_sec:
            tbl.append(f"·{sec[:10]}·")
            cur_sec = sec
        tbl.append(" | ".join(_j(i, r[i]) for i in range(len(hdrs))))
    tbl.append(sep)
    tbl.append("[+]=UP [!]=DN [ ]=FLAT")

    pre_block = "<pre>" + "\n".join(tbl) + "</pre>"

    # ── Mobile cards with section headers (primary display on phone) ─
    colour_lines = []
    cur_sec = ""
    for sec, name, d, tag in all_rows:
        if sec != cur_sec:
            colour_lines.append(f"\n<b>── {sec} ──</b>")
            cur_sec = sec
        if d:
            note = f"  <i>{tag}</i>" if tag else ""
            colour_lines.append(
                f"{d['em']} <b>{name}</b>  {d['px_s']}  <b>{d['chg']:+.2f}%</b>{note}")
        else:
            colour_lines.append(f"⚪ <b>{name}</b>  N/A")
    colour_block = "\n".join(colour_lines)
...
async def exit_planner_menu(query):
    ...
    if open_trades.empty:
        ...
        btns = [[InlineKeyboardButton(f"🎯 {t}", callback_data=f"exit_{t}_put_290_5.2_2026-03-20") for t in tickers[:3]]]
        btns.append([InlineKeyboardButton(f"🎯 {t}", callback_data=f"exit_{t}_put_290_5.2_2026-03-20") for t in tickers[3:]])
        btns.append([BACK_BTN])
        await query.message.reply_text(
    ...
    for _, tr in open_trades.iterrows():
        ...
        ep = tr["entry_price"]
        ex = tr["expiry"]
        label = f"🎯 {tk} {ot} ${st:.0f} (entry ${ep:.2f})"
        data = f"exit_{tk}_{ot.lower()}_{st}_{ep}_{ex}"
...
async def run_exit_analysis(query, ticker, opt_type, strike, entry, expiry_str):
    ...
    # Vectorized BS
    sqrt_T = np.sqrt(T_tomorrow)
    r = 0.045
    K = float(strike)
    d1 = (np.log(sim_prices / K) + (r + 0.5 * sim_ivs**2) * T_tomorrow) / (sim_ivs * sqrt_T)
    d2 = d1 - sim_ivs * sqrt_T
    # Price the simulated paths using Black-Scholes (vectorized via list comprehension)
    try:
        option_vals = np.array([
            bs_price(s, K, T_tomorrow, r, sigma, opt=opt_type)
            for s, sigma in zip(sim_prices, sim_ivs)
        ])
    except Exception:
        option_vals = np.zeros_like(sim_prices)
    ...
    prob_profit = float(np.mean(option_vals > float(entry)) * 100.0) if len(option_vals) else 0.0
    ...
    pnl_pct = (exp_val - float(entry)) / float(entry) * 100 if float(entry) > 0 else 0
    if prob_profit > 55 and pnl_pct > 10:
        rec = "<b>🟢 SET LIMIT SELL — Take Profit</b>"
        rec_detail = f"MC: {prob_profit:.0f}% profit chance. Target ${entry * 1.3:.2f} (+30%)"
    elif prob_profit > 55:
        rec = "<b>🟡 HOLD WITH STOP</b>"
        rec_detail = f"MC: {prob_profit:.0f}% profit chance. Stop at ${entry * 0.80:.2f}"
    elif prob_profit > 40:
        rec = "<b>🟠 TIGHT STOP-LOSS</b>"
        rec_detail = f"MC: {prob_profit:.0f}% profit. Stop at ${entry * 0.80:.2f}, VaR: ${var_95:+,.0f}"
    else:
    ...
    msg = (
        f"{hdr(f'🎯 {ticker} {opt_type.upper()} ${K:.0f}')}\n\n"
        ...
        + mono(
            ...
            f"{'─' * 27}\n"
            f"{pnl_emoji} {row2('Exp. P&L', f'${exp_pnl:+,.0f} ({pnl_pct:+.0f}%)')}\n"
            f"{row2('P(Profit)', f'{prob_profit:.0f}%  {profit_bar}')}\n"
            f"{row2('VaR 95%', f'${var_95:+,.0f}')}"
        )
        + "\n📊 <b>Greeks (Current)</b>\n"
        + mono(
            f"{row2('Theo Value', f'${cur_val:.2f}')}\n"
            f"{row2('Delta', f'{greeks['delta']:.3f}')}\n"
            f"{row2('Theta', f'-${abs(greeks['theta'])*100:.2f}/day')}\n"
            f"{row2('Vega', f'${greeks['vega']*100:.2f}')}"
        )
        + f"\n💡 <b>Recommendation</b>\n{rec}\n{rec_detail}\n"
        + f"\n<i>Updated {datetime.now().strftime('%H:%M:%S')}</i>"
    )

    cb_data = f"exit_{ticker}_{opt_type}_{strike}_{entry}_{expiry_str}"
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Refresh", callback_data=cb_data)],
        [InlineKeyboardButton("📊 Scenarios", callback_data=f"scenarios_{ticker}_{opt_type}_{strike}_{entry}_{expiry_str}")],
...
async def show_scenarios(query, ticker, opt_type, strike, entry, expiry_str):
    ...
    tbl_rows = [f"{'Move':>5}  {'Stock':>6}  {'Value':>6}  {'ST':<3} {'P&L':>7}"]
    tbl_rows.append("─" * 33)
    for mv in moves:
        s = spot * (1 + mv / 100)
        ...
        pnl = (val - entry) * 100
        badge = "[+]" if pnl >= 0 else "[-]"
        tbl_rows.append(f"{mv:>+5.1f}%  ${s:>5.0f}  ${val:>5.2f}  {badge} ${abs(pnl):>5,.0f}")

    parts = [
        hdr(f"📊 SCENARIOS · {ticker} {opt_type.upper()} ${K:.0f}"),
        mono(
...
async def mirofish_ticker_detail(query, ticker):
    ...
    if not sd.empty:
        ...
        parts.append(mono(
            f"{'Close':<10} ${close:.2f}\n"
            f"{'PCR':<10} {pcr:.2f}  {pcr_bias_text}"
        ))
        parts.append(f"{pcr_em} PCR Bias: <b>{pcr_bias_text}</b>")

    if not oc.empty:
        # Top 5 strike moves
        parts.append("\n📊 <b>Top Strike Moves:</b>")
        ...
        if r1 > 0 and s1 > 0:
            parts.append(f"\n🎯 <b>Key Levels:</b>  R1 <b>${r1:.1f}</b>  ·  S1 <b>${s1:.1f}</b>")
        parts.append(f"\n📌 <b>Bias:</b> {bias}")
        parts.append(f"💡 <b>Suggestion:</b> {rec}")
...
async def oi_menu(query, expiry=None):
    ...
    # Step 2: Get distinct expiry_date values from the latest trade_date
    try:
        all_exp_df = pd.read_sql("""
            SELECT DISTINCT expiry_date FROM options_daily WHERE trade_date = ?
    ...
    # Default expiry: nearest future, or most recent expired if no future
    chosen_expiry = expiry if expiry else (future_dates[0] if future_dates else (expired_dates[0] if expired_dates else None))
...
def _generate_oi_change_chart(ticker, today_date, prev_date):
    ...
    try:
        ...
        for idx, expiry in enumerate(all_expiries):
            ...
            # Yesterday's OI (base, low alpha)
            ax.bar(strikes, df["openInt_Call_prev"], width=width, alpha=0.28, color='#43A047', label='Call Prev')
            ax.bar(strikes, -df["openInt_Put_prev"], width=width, alpha=0.28, color='#E53935', label='Put Prev')

            # Today's OI (overlay, higher alpha)
            ax.bar(strikes, df["openInt_Call"], width=width, alpha=0.75, color='#1B5E20', label='Call Today')
            ax.bar(strikes, -df["openInt_Put"], width=width, alpha=0.75, color='#B71C1C', label='Put Today')

            ax.axhline(y=0, color='#212121', linestyle='-', linewidth=0.8)
            ax.set_title(f"{ticker}  |  Expiry: {expiry}\nPrev: {prev_date}  →  Today: {today_date}",
                         fontsize=10, fontweight="bold")
            ax.set_ylabel('Open Interest')

            # Strike labels on x-axis — max 14 labels, rotated
            ...
            # Legend — upper LEFT so signal box on right stays clear
            ax.legend(loc='upper left', fontsize=7, ncol=2, framealpha=0.75)
            ...
            pcr_prev = (df["openInt_Put_prev"].sum() / df["openInt_Call_prev"].sum()) if df["openInt_Call_prev"].sum() > 0 else 0
            pcr_today = (total_put_oi / total_call_oi) if total_call_oi > 0 else 0
            pcr_change = pcr_today - pcr_prev
            
            # Hedge-aware intent signal (replaces old sentiment logic)
            ...
            if _spot and len(df) >= 2:
                df["call_oi_change"] = df["openInt_Call_chg"]
                df["put_oi_change"]  = df["openInt_Put_chg"]
                _, _isig, _isc, _idesc, _idet = _oi_intent_algo(df, _spot)
                sig_txt    = f"SIGNAL: {_isig}\n{chr(8212)*20}\n{_idesc}"
                hedge_note = f"  Hedge%: {_idet.get(chr(104)+chr(101)+chr(100)+chr(103)+chr(101)+chr(95)+chr(112)+chr(99)+chr(116),0):.0f}%"
                sig_col    = _isc
            else:
                _msig, _msc = _oi_signal_light(total_call_chg, total_put_chg, pcr_today)
                sig_txt    = f"SIGNAL: {_msig}"
                hedge_note = ""
                sig_col    = _msc

            # Ensure pcr_eod is defined, fallback to 0.0 if not available
            # Only use pcr_today or 0.0 for PCR display
            _pcr_val = pcr_today if 'pcr_today' in locals() else 0.0
            ax.text(0.02, 0.04,
                    f"OI CHANGES\nCall: {total_call_chg:+,.0f}  ({call_pct_chg:+.1f}%)\n"
                    f"Put:  {total_put_chg:+,.0f}  ({put_pct_chg:+.1f}%)\n"
                    f"PCR:  {_pcr_val:.2f}{hedge_note}",
            ...
            ax.text(0.99, 0.98, sig_txt, transform=ax.transAxes,
                    va="top", ha="right", fontsize=8, fontweight="bold", color="white",
                    bbox=dict(boxstyle="round,pad=0.5", facecolor=sig_col,
...
async def market_analytics_report(query):
    ...
    _f_RIGHT = {2, 3}
    _f_data  = []
    _f_colour = []   # emoji HTML lines for mobile primary display
    sentiment_score = 0
    vix_val = 0
    for name, sym in fut_syms:
        try:
            ...
            if len(h) >= 2:
                ...
                _f_data.append([st, name, px_s, f"{chg:+.2f}%"])
                _f_colour.append(f"{em} <b>{name}</b>  {px_s}  <b>{chg:+.2f}%</b>")
            else:
    ...
    if _f_data:
        _fw  = [max(len(_f_hdrs[i]), max(len(r[i]) for r in _f_data)) for i in range(len(_f_hdrs))]
        _fj  = lambda i, v: v.rjust(_fw[i]) if i in _f_RIGHT else v.ljust(_fw[i])
        _fsep = "-+-".join("-" * w for w in _fw)
        _flines = [" | ".join(_fj(i, _f_hdrs[i]) for i in range(len(_f_hdrs))), _fsep]
        for r in _f_data:
            _flines.append(" | ".join(_fj(i, r[i]) for i in range(len(_f_hdrs))))
    ...
    # OI summary block
    oi_sum = [f"OI SIGNALS  {latest_date}", "─" * 29]
    if bull_tickers:    oi_sum.append(f"Bull  {', '.join(bull_tickers[:5])}")
    if bear_tickers:    oi_sum.append(f"Bear  {', '.join(bear_tickers[:5])}")
    if unusual_tickers: oi_sum.append(f"Mix   {', '.join(unusual_tickers[:4])}")
    parts.append(mono("\n".join(oi_sum)))

    # OI flow cards — HTML format wraps naturally on mobile
    def _oi_table(label, data, badge_em):
        if not data: return ""
        lines = [f"<b>{label}</b>"]
        for tk, c, p, pcr in data[:6]:
            c_s = _fmt_k(c); p_s = _fmt_k(p)
            lines.append(f"{badge_em} <b>{tk}</b>  C:{c_s}  P:{p_s}  PCR:{pcr:.1f}")
        return "\n".join(lines)

    if bull_rows_data:
        parts.append(_oi_table("🟢 Call OI Building", bull_rows_data, "🟢"))
    if bear_rows_data:
        parts.append(_oi_table("🔴 Put OI Building", bear_rows_data, "🔴"))
    ...
    try:
        ...
        for sym in _scan:
            try:
                ...
                em  = "🟢" if sig == "BULL" else ("🔴" if sig == "BEAR" else "🟡")
                rsi_em = "🔻" if rsi_v < 30 else ("🔺" if rsi_v > 70 else "")
                macd_em = "↑" if cd == "BUY" else "↓"
                _t_lines.append(
                    f"{em} <b>{sym}</b>  [{sig}] {bull_pts}/5\n"
                    f"   RSI:{rsi_v:.0f}{rsi_em} MACD:{cd}{macd_em} BB:{bb_pos} EMA:{ema_pct:+.1f}%"
                )
            except Exception:
                continue
        if _t_lines:
            parts.append("<b>📐 TECH  RSI·MACD·BB·EMA</b>\n" + "\n".join(_t_lines))
...
async def position_monitor(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Single-char status badge (keeps table narrow) ─────────────
    def _st_badge(action, pnl_pct):
        if action == "EXIT NOW":                       return "!"
        if action == "CUT LOSS"  and pnl_pct <= -50:  return "X"
        if action == "CUT LOSS":                       return "-"
        if action == "TAKE PROFIT" and pnl_pct >= 70: return "*"
        if action == "TAKE PROFIT":                    return "+"
        if action in ("ROLL/EXIT", "ROLL SOON"):       return ">"
        if action == "REVIEW":                         return "?"
        return "."

    # 4-char action abbreviation to keep table width ≤ 33 chars
    def _act_abbr(action):
        return {"EXIT NOW": "EXIT", "CUT LOSS": "CUT!", "TAKE PROFIT": "PROF",
                "ROLL/EXIT": "ROLL", "ROLL SOON": "ROLL", "REVIEW": "RVEW"}.get(action, "HOLD")

    # 5-column narrow pre table: S|Tkr|Strk|P&L%|Act — target ≤33 chars wide
    hdrs  = ["S", "Tkr", "Strk", "P&L%", "Act"]
    RIGHT = {2, 3}

    str_rows   = []
    html_cards = []   # per-position full detail in HTML below pre block
    ...
    for (em, tk, otype, strike, entry, cur_px, pnl_pct, pnl, dte_s, prob_s, oi_s, action) in rows:
        ...
        pct_s = f"{pnl_pct:+.1f}%"
        st    = _st_badge(action, pnl_pct)
        act   = _act_abbr(action)
        str_rows.append([st, tk[:5], str(int(strike)), pct_s, act])
        # Full detail in HTML card (wraps OK — it's descriptive text)
        pnl_s = f"{pnl:+,.0f}"
        html_cards.append(
            f"{em} <b>{tk}</b> {otype} ${int(strike)}\n"
            f"   Buy:{buy_s}  Now:{cur_s}  <b>{pct_s}</b> ({pnl_s})\n"
            f"   {dte_s}  Prob:{prob_s}  → <b>{action}</b>"
        )

    # Dynamic col widths — Strk and P&L% right-justified
    col_w = [
        max(len(hdrs[i]), max(len(r[i]) for r in str_rows) if str_rows else len(hdrs[i]))
        for i in range(len(hdrs))
    ]
    _J = lambda i, v: v.rjust(col_w[i]) if i in RIGHT else v.ljust(col_w[i])

    sep      = "-+-".join("-" * w for w in col_w)
    hdr_line = " | ".join(_J(i, hdrs[i]) for i in range(len(hdrs)))

    tbl_lines = [f"POSITIONS {now_s}", hdr_line, sep]
    for r in str_rows:
        tbl_lines.append(" | ".join(_J(i, r[i]) for i in range(len(hdrs))))
    tbl_lines.append(sep)

    net_sign = "+" if total_pnl >= 0 else ""
    tbl_lines.append(f"P&L: {net_sign}{total_pnl:,.0f}  ({len(str_rows)} pos)")
    tbl_lines.append("S: !=EXIT X=CUT(lg) -=CUT +=PROF *=WIN! >=ROLL ?=RVEW")

    # ── Assemble: pre block + emoji colour summary ────────────────
    pre_block = "<pre>" + "\n".join(tbl_lines) + "</pre>"
    ...
    footer  = f"\n{net_em} <b>Portfolio: ${total_pnl:+,.0f}</b>  ({len(str_rows)} positions)"

    full_msg = pre_block + "\n\n" + colour_section + urgent_section + footer
    ...
    try:
        # split if > 4000 chars (Telegram limit 4096)
        if len(full_msg) <= 4000:
            await ctx.bot.send_message(chat_id=int(chat_id), text=full_msg,
                                       parse_mode=H, reply_markup=kb)
        else:
            await ctx.bot.send_message(chat_id=int(chat_id), text=pre_block, parse_mode=H)
            await ctx.bot.send_message(chat_id=int(chat_id),
                                       text=colour_section + urgent_section + footer,
                                       parse_mode=H, reply_markup=kb)
    except Exception as e:
        log.warning(f"position_monitor send failed: {e}")


async def position_monitor_adhoc(query):
    """On-demand position monitor triggered by button press."""
    await query.answer("Fetching live positions…")
    # Reuse position_monitor logic by constructing a mock context carrier
    class _MockCtx:
        ...
        if not trades.empty:
            ...
            if not latest_row.empty:
                    ...
                    try:
                        ...
                        df = pd.read_sql("""
                            SELECT expiry_date,
                                   ...
                                   strike,
                                   MAX(ABS(change_OI_Call) + ABS(change_OI_Put)) AS top_move
                            FROM options_change
                        ...
                        if not df.empty:
                            ...
                            top_df = pd.read_sql("""
                                ...
                                ORDER BY (ABS(change_OI_Call)+ABS(change_OI_Put)) DESC LIMIT 1
                            """, conn, params=(tk, latest_dt, str(r["expiry_date"])))
                            if not top_df.empty:
                                tr   = top_df.iloc[0]
                                c2   = float(tr["change_OI_Call"] or 0)
                                p2   = float(tr["change_OI_Put"]  or 0)
                                st2  = float(tr["strike"] or 0)
                                mtyp = "Call OI" if abs(c2) > abs(p2) else "Put OI"
                                mval = c2 if abs(c2) > abs(p2) else p2
                                key_moves.append(f"{tk}: {mtyp} {mval:+,.0f} @ ${st2:.0f}")
                    except Exception:
                        pass
                ...
                if _oi_data:
                    ...
                    parts.append("\n" + "\n".join(_oi_lines))
                if key_moves:
                    parts.append("\n<b>KEY MOVES THIS SESSION:</b>")
                    for m in key_moves[:5]:
                        parts.append(f"  {m}")
    except Exception as e:
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu_main":
            ...
            parts = data.split("_")
            # exit_TICKER_type_strike_entry_expiry
            ticker = parts[1]
            opt_type = parts[2]
            strike = float(parts[3])
            entry = float(parts[4])
            expiry_str = parts[5]
            await run_exit_analysis(query, ticker, opt_type, strike, entry, expiry_str)
            ...
            await position_monitor_adhoc(query)
        ...
        # ── Group / Strategy Builder ──────────────────────────────
            ...
            await query.message.reply_text(
                ...
                f"<code>http://localhost:8501</code>\n\n"
                f"• On your <b>PC</b>: <code>http://localhost:8501</code>\n"
                f"• On your <b>phone</b>: Use your PC's local IP address and port 8501\n\n"
                f"<b>Pages:</b>\n"
...
async def morning_alert(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Market overview
    # Market rows: 3 short-name columns — Name(3)|Price(9)|Chg%(7) = ~23 chars
    _mkt_specs = [("ES", "ES=F"), ("NQ", "NQ=F"), ("VIX", "^VIX")]
    _mkt_hdrs  = ["Name", "Price", "Chg%"]
    _mkt_data  = []
    colour_mkt = []
    for short, sym in _mkt_specs:
        try:
            ...
            if len(h) >= 2:
                ...
                em  = "🟢" if pct > 0 else "🔴" if pct < 0 else "⚪"
                _mkt_data.append([short, f"{px:,.2f}", f"{pct:+.2f}%"])
                colour_mkt.append(f"{em} {short}: {px:,.2f}  {pct:+.2f}%")
        except Exception:
            pass
    if _mkt_data:
        _cw = [max(len(_mkt_hdrs[i]), max(len(r[i]) for r in _mkt_data)) for i in range(3)]
        _R  = {1, 2}
        _jn = lambda i, v: v.rjust(_cw[i]) if i in _R else v.ljust(_cw[i])
        _sep = "-+-".join("-"*w for w in _cw)
        mkt_rows = [" | ".join(_jn(i, _mkt_hdrs[i]) for i in range(3)), _sep]
        for r in _mkt_data:
            mkt_rows.append(" | ".join(_jn(i, r[i]) for i in range(3)))
        parts.append(mono("\n".join(mkt_rows)))
        parts.append("\n".join(colour_mkt))
    ...
    if not open_trades.empty:
        parts.append("\n💼 <b>Open Positions</b>")
        pos_rows = [f"{'ST':<3} {'Tkr':<5} {'Tp':<4} {'Stk':>4} {'PnL':>5}"]
        pos_rows.append("─" * 25)
        colour_pos = []
        for _, tr in open_trades.iterrows():
            tk = str(tr["ticker"])[:5]
            ...
            try:
                ...
                badge = "[+]" if pnl_pct > 0 else "[-]"
                em = "🟢" if pnl_pct > 0 else "🔴"
                combo = f"{side_s}{ot}"
                pos_rows.append(f"{badge} {tk:<5} {combo:<4} {st:>4.0f} {pnl_pct:>+4.0f}%")
                colour_pos.append(f"{em} {tr['ticker']} {ot} ${st:.0f} | P&L {pnl_pct:+.1f}%")
            except Exception:
                combo = f"{side_s}{ot}"
                pos_rows.append(f"[ ] {tk:<5} {combo:<4} {st:>4.0f}  n/a")
                colour_pos.append(f"⚪ {tr['ticker']} {ot} ${st:.0f}")
        parts.append(mono("\n".join(pos_rows)))
        parts.extend(colour_pos)
...
async def overnight_risk_report(query):
    ...
    for idx, tr in trades.iterrows():
        ...
        try:
            h    = yf.Ticker(tk).history(period="5d")
            spot = float(h["Close"].iloc[-1]) if len(h) >= 1 else strk
        except Exception:
            spot = strk

        T = max(dte, 1) / 365.0
        greeks = bs_greeks(spot, strk, T, r_rate, iv_base, opt=ot.lower())
        theo   = bs_price(spot, strk, T, r_rate, iv_base, opt=ot.lower())

        delta = greeks.get("delta", 0)
        theta = greeks.get("theta", 0)  # per day in dollar per share

        contracts = abs(qty)
        pos_sign  = 1 if qty > 0 else -1

        theta_day   = theta * 100 * contracts * pos_sign         # $ theta overnight
        delta_1pct  = delta * spot * 0.01 * 100 * contracts * pos_sign  # $ P&L if +1%
        ...
        # Risk level
        pnl_entry = (theo - entry) / entry * 100 if entry > 0 else 0
        risk_lvl = "HIGH" if dte <= 3 or pnl_entry < -40 else ("MED" if dte <= 7 else "LOW")

        risk_rows.append([
            tk, ot[:4], f"{strk:.0f}", str(dte)+"d",
            f"${theta_day:+.0f}",
            f"${delta_1pct:+.0f}",
            risk_lvl
        ])
    ...
    gap_pnl = total_delta_1pct * -2   # simple linear approximation

    buf = _tbl_img(
        f"OVERNIGHT RISK  {datetime.now().strftime('%H:%M ET')}",
        ["Tkr", "Type", "Strk", "DTE", "Theta/day", "Delta(1%)", "Risk"],
        risk_rows,
        right_cols={2, 3, 4, 5},
        highlight=hl,
        subtitle=(f"Total Theta tonight: ${total_theta_day:+,.0f}  |  "
                  f"Gap-down 2% scenario: ${gap_pnl:+,.0f}  |  "
                  f"VIX: {vix_val:.1f}")
    )
    ...
    summary = (
        ...
        f"<i>Red = HIGH risk (DTE≤3 or P&L≤-40%)</i>"
    )
    ...
    await query.message.reply_text(summary, parse_mode=H)
    await query.message.reply_photo(buf, caption="Per-position risk detail", reply_markup=kb)
    try: await _loading.delete()
    except Exception: pass
