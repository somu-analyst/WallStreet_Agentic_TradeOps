...
async def run_exit_analysis(query, ticker, opt_type, strike, entry, expiry_str, qty=1):
    ...
    try:
        for sym, label in [("ES=F", "ES"), ("NQ=F", "NQ")]:
            fh = _yf_ticker(sym).history(period="5d")
            if len(fh) >= 2:
...
def _parent_child_payoff(parent_trade, child_cfg):
    ...
    spot = 100.0
    try:
        h = _yf_ticker(tk).history(period="5d")
        if len(h) >= 1:
            spot = float(h["Close"].iloc[-1])
    except Exception:
        log.debug("suppressed exception", exc_info=True)
...
async def oi_detail(query, ticker):
    ...
    try:
        tk_obj = _yf_ticker(str(ticker).upper())
...
def _generate_oi_change_chart(ticker, today_date, prev_date):
    ...
    try:
        ...
        for idx, expiry in enumerate(all_expiries):
            ...
            # ── Intent signal ──────────────────────────────────────────
            try:
                _spot = float(_yf_ticker(ticker).history(period="2d")["Close"].iloc[-1])
            except Exception:
                _spot = None
...
def _generate_live_vs_eod_chart(ticker, live_data_list, eod_date):
    ...
    try:
        _sh  = _yf_ticker(ticker).history(period="2d")
...
async def position_alerts(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for grp_key, grp in trades.groupby("_grp"):
        ...
        stock_px = None
        try:
            _sh = _yf_ticker(tk).history(period="1d", interval="5m")
            if not _sh.empty:
                stock_px = float(_sh["Close"].iloc[-1])
        except Exception:
            log.debug("suppressed exception", exc_info=True)
        ...
        for _, tr in grp.iterrows():
            ...
            try:
                ...
                _chain = _yf_ticker(tk).option_chain(_exp_yf)
                _df    = _chain.calls if otype == "CALL" else _chain.puts
                _near  = _df[abs(_df["strike"] - strike) < 0.01]
...
async def intraday_alert(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for name, sym in fut_symbols:
        try:
            h = _yf_ticker(sym).history(period="1d", interval="5m")
            if len(h) >= 2:
                px  = float(h["Close"].iloc[-1])
    ...
    try:
        ...
        for sym in _watch[:8]:
            try:
                _h = _yf_ticker(sym).history(period="10d", interval="1d")
                if len(_h) < 6:
                    continue
            ...
            if not latest_row.empty:
                ...
                for tk in tickers:
                    try:
                        ...
                        spot_tk = 0.0
                        try:
                            _th = _yf_ticker(tk).history(period="2d")
                            if len(_th) >= 1:
...
async def live_predictor_view(query):
    try:
        es = _yf_ticker("ES=F").history(period="5d")
        vix = _yf_ticker("^VIX").history(period="5d")
...
async def morning_alert(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not open_trades.empty:
        ...
        for _, tr in open_trades.iterrows():
            ...
            try:
                _h5 = _yf_ticker(tr["ticker"]).history(period="7d")
    ...
    try:
        ...
        try:
            _jc_h = _yf_ticker("^IRX").history(period="10d")
            if len(_jc_h) >= 2:
        ...
        # DXY — dollar index
        try:
            _dxy_h = _yf_ticker("DX=F").history(period="5d")
            if len(_dxy_h) >= 2:
        ...
        # 10Y yield
        try:
            _tnx_h = _yf_ticker("^TNX").history(period="5d")
            if len(_tnx_h) >= 2:
        ...
        # Crude Oil
        try:
            _oil_h = _yf_ticker("CL=F").history(period="5d")
            if len(_oil_h) >= 2:
        ...
        # VIX vol regime
        try:
            _vix_h = _yf_ticker("^VIX").history(period="5d")
            if len(_vix_h) >= 1:
...
def short_squeeze_signal(ticker, conn=None):
    ...
    try:
        h = _yf_ticker(tk).history(period="40d")
        if len(h) >= 5:
...
# ============================================================================
#  MERGED-IN DEFINITIONS (were only in the now-deleted second copy)
# ============================================================================
...
def _tg_spot_label(ticker: str) -> tuple:
    """Returns (best_price, display_label, source_key) using live/AH/EOD.
    source_key: 'ah' | 'pm' | 'live' | 'eod' | 'wkd'
    Use best_price for all calculations; display_label for Telegram output.
    """
    from datetime import datetime as _dt
    try:
        import pytz as _tz
        _now = _dt.now(_tz.timezone("US/Eastern"))
        _wday = _now.weekday()  # 0=Mon 6=Sun
        _hour = _now.hour + _now.minute / 60
    except Exception:
        _wday, _hour = 0, 12.0

    ah = _get_spot_with_ah(ticker)
    reg = ah["spot_reg"]
    ext = ah["spot_ext"]
    src = ah["ext_src"]
    is_ext = ah["is_extended"]
    chg = ah["ext_chg_pct"]

    if _wday >= 5:  # Weekend
        lbl = f"📅 WKD ${reg:.2f}" if reg > 0 else "📅 N/A"
        return reg, lbl, "wkd"
    if is_ext and ext > 0 and abs(ext - reg) > 0.01:
        if "Post" in src or "post" in src:
            return ext, f"🌙 AH ${ext:.2f} ({chg:+.1f}% vs EOD ${reg:.2f})", "ah"
        else:
            return ext, f"🌅 PM ${ext:.2f} ({chg:+.1f}% vs EOD ${reg:.2f})", "pm"
    if 9.5 <= _hour <= 16.0 and _wday < 5:
        return reg, f"📈 Live ${reg:.2f}", "live"
    return reg, f"☀️ EOD ${reg:.2f}", "eod"
...
async def recommend_engine(query):
    ...
    parts.append(
        f"\n<i>Scored {len(scored)} tickers | VIX {vix_val:.1f} | "
        f"{datetime.now().strftime('%H:%M')} ET</i>"
    )

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Refresh",       callback_data="menu_recommend"),
         InlineKeyboardButton("🌍 Global Market", callback_data="menu_global_market")],
        [InlineKeyboardButton("📡 Market",        callback_data="menu_analytics"),
         InlineKeyboardButton("📊 OI Signals",    callback_data="menu_signals")],
        [BACK_BTN],
    ])
    try:
